...
- 文(Statement): 何らかの手続きを処理系に命令するもの
  - 変数に代入できない
  - if, for、関数宣言など
  - 文の末尾にはセミコロンをつける
  - 文は式になれない
...
- プロトタイプチェーン: プロトタイプを参照する仕組み。クラス内部→プロトタイプの順で探索
  - インスタンス作成時: prototypeへの参照経路を保存
  - プロパティorメソッド呼び出し時: prototypeを参照
...
- throwでエラーを投げることができる
  - デバッグ情報を持つErrorオブジェクトのインスタンスを投げるのを推奨
  - ビルトインエラー: jsに元々組み込まれたエラー処理。全てErrorオブジェクトのインスタンス
  - 開発中にエラーが発生したら、MDNリファレンスでそのエラーを確認すると良い

## 同期、非同期処理

- 同期処理: 一つの処理を順番に実行。一つずつしか実行できない
- 非同期処理: 並列実行

- なぜ非同期処理をする必要があるのか？
  - JSの同期処理はメインスレッドで実行される。ブラウザでのメインスレッド(UIスレッド)ではUIの描画や表示の更新を行うため、メインスレッドがブロックされると画面の更新が止まり、フリーズしたようになってしまうため

  非同期処理もメインスレッドで行われている(非同期関数の中から外側のスコープを参照できる)

  WebWorker APIを使うとスレッドを分けた非同期処理もできる

### 非同期処理でのエラーキャッチ方法

try-catchのtryの中では、同期処理で起こった例外しかcatchできない。(非同期処理の外から、非同期処理の中で発生した例外をキャッチできない)

- エラーファーストコールバック: コールバック関数への引数の渡し方でエラーかどうか判定
  - エラーなら第一引数にErrorオブジェクト、そうでなければnull
  - Node.jsでは標準

### Promise

- エラーファーストコールバックのやり方をビルトインオブジェクト（Promiseオブジェクト）にして、改善
- Promiseの状態
  - Pending: インスタンス作成時の状態
  - settled
    - Fullfilled: 成功時
    - Rejected: エラー発生時
- プロミスチェーン: thenをつなげて書くことで、非同期処理を順次実行できる
- Promiseの問題点: 例外処理を簡単に書くために導入されたが、非同期処理を順次実行したい時などは結局複雑になる

### async, await

- Promiseをより簡単に書くために導入された
- async function: 必ずPromiseインスタンスを返す関数
- await: 非同期処理の順番待ちを可能にする
- async, awaitを組み合わせて非同期処理を同期処理のような形式で書ける

## Node.js
- OSを問わずクロスプラットフォームで開発可能
- LTS版(安定)と最新版の二つがあり、LTS版が使われることが多い
- npm: Node.js付属のパッケージマネージャ
  - npxコマンド: npmで公開されている実行可能なパッケージのインストールと実行をまとめて行う
